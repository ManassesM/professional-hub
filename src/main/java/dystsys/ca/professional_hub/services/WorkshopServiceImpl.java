...
public class WorkshopServiceImpl extends WorkshopImplBase {
	...
	if (!MockDB.REGISTERED_WORKERS.contains(workerId)) {
	    responseObserver.onError(
		    Status.NOT_FOUND.withDescription("Worker ID " + workerId + " was not found on database.").asRuntimeException());
	    return;
	}
	...
	if (!MockDB.WORKSHOP_NOTES.containsKey(workshopId)) {
	    responseObserver
		    .onError(Status.NOT_FOUND.withDescription("Workshop ID " + workshopId + " does not exist.").asRuntimeException());
	    return;
	}
