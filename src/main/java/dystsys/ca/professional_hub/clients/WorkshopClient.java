...
public class WorkshopClient {
	...
	public static void main(String[] args) {
		...
		DiscoveredService ds = ServiceDiscovery.findService("_workshop._tcp.local.");
	...
	private static void checkInWorkerCall(String workerId, String workshopId) {
		...
		CheckInWorkerReq request = CheckInWorkerReq.newBuilder().setWorkerId(workerId).setWorkshopId(workshopId)
				.build(); // builds and
		// sends request
		try {

			CheckInWorkerRes response = stub.checkInWorker(request); // gets response
			System.out.printf("Worker %s checked in!%nModule name: %s%nCredits: %d%n", workerId,
					response.getModuleName(), response.getWorkshopCredits());
		...
		StreamObserver<RangeCheckRes> responseObserver = new StreamObserver<RangeCheckRes>() {
			...
			public void onNext(RangeCheckRes response) {
				try {
					...
					System.out.printf("Signal received!%nProximity Status: %s%nStatus: %s%n", proximity_status,
							status_message);
